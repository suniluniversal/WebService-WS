...
// Model for ContactPerson entity
    models.ContactPerson = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/WebService-WS/webresources/com.entity.contactperson/",
        idAttribute: 'contactPersonId',
        defaults: {
            phoneNumber: "",
            name: ""
        },
        ...
        sync: function (method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:8080/WebService-WS/webresources/com.entity.contactperson/';
            }
    ...
    // Collection class for ContactPerson entities
    models.ContactPersonCollection = Backbone.Collection.extend({
        model: models.ContactPerson,
        url: "http://localhost:8080/WebService-WS/webresources/com.entity.contactperson/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.ContactPersonCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-contactperson-list-itemis template identifier for item
                        templateName: '#tpl-contactperson-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.contactperson = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.contactperson,
                    // tpl-contactperson-details is template identifier for chosen model element
                    templateName: '#tpl-contactperson-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.ContactPerson();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-contactperson-details is a template identifier for chosen model element
                templateName: '#tpl-contactperson-details',
